...
/** Compiles generated tupson sources with dotc to make sure they are valid. */
class GithubJiraCompilationSuite extends munit.FunSuite {

  // dotc compiling the ~800 github models takes ~25 minutes, so the default 30s timeout is not enough
  override def munitTimeout: Duration = Duration(45, "min")

  test("generated tupson sources compile for github.json") {
    val baseFolder = generate("github.json", "com.example.github")
    CompilationTestUtils.compileGenerated(baseFolder.resolve("com/example/github"))
  }

  test("generated tupson sources compile for jira-cloud-v3.json") {
    val baseFolder = generate("jira-cloud-v3.json", "com.example.jira")
    CompilationTestUtils.compileGenerated(baseFolder.resolve("com/example/jira"))
  }
