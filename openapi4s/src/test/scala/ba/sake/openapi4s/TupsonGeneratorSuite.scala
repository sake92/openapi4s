...
class TupsonGeneratorSuite extends munit.FunSuite {
  ...
  test("tupson generator should generate modern schema constructs") {
    ...
    assert(containerFile.contains("meta: (kind: String, age: Int)"), s"anonymous object not a named tuple:\n$containerFile")
    assert(containerFile.contains("nested: (inner: (x: String))"), s"nested anonymous object not a named tuple:\n$containerFile")
    ...
    assert(withMapFile.contains("freeForm: Map[String, JValue]"), s"free-form map not Map[String, JValue]:\n$withMapFile")
